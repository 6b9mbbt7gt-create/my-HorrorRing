            ...
            <div className="flex items-start justify-between mb-2">
              <div className="flex-1">
                <p className="text-sm text-gray-400">
                  {getAuthorDisplayName(comment.userId, authorNames?.[comment.userId])} • {formatRelativeTime(comment.createdAt)}
                </p>
              </div>
              <button
                ...
                {getReplies(comment.id).map((reply) => (
                  <div key={reply.id} className="bg-gray-800 rounded p-3">
                    <p className="text-xs text-gray-400 mb-1">
                      {getAuthorDisplayName(reply.userId, authorNames?.[reply.userId])} • {formatRelativeTime(reply.createdAt)}
                    </p>
